...
export default function Profile() {
  ...
  async function getProfile() {
    try {
      const response = await fetch(
        `http://localhost:5000/getProfile?id=${Cookies.get("userId")}`,
        {
          method: "GET",
          headers: {
  ...
  async function handleDelete() {
    if (window.confirm("Are you sure you want to deactivate you account?")) {
      setdSubmit(true);
      await fetch(
        `http://localhost:5000/deleteuser?id=${Cookies.get("userId")}`,
        {
          method: "DELETE",
          headers: {
            "api-key": process.env.REACT_APP_API_KEY,
        ...
        <center>
          <MDBCol md="7">
            <MDBCard className="my-5">
              <form onSubmit={handleSubmit} method="post" id="addform">
