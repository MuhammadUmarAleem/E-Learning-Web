          ...
          if (res.length == 0) {
            connection.query("INSERT INTO users SET ?", data, (err, res) => {
              if (err) {
                ...
                const body = `<p>Dear User!<p><p> Thanks for you interest in Wise Way. To Create Your Account verify you email. You Verification Code is <br/> <center><h1>${code}</h1></center>`;
                async function send() {
